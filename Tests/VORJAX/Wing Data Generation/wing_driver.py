...
def wing_generator(df_geometries):
    ...
    for row in df_geometries.itertuples(index=False):
        ...
        wing = Wing(
            ...
            spans=WingDimensions(projected=row.aspect_ratio * (1 + row.taper)/2),
            origin=jnp.array([[0., 0., 0.]]),
        ).update_geometry(calculate_reference_area=True, calculate_wetted_area=True)

        system = Aircraft(tag=f"{row.wing_ID}_System", areas=wing.areas).add_subcomponent(wing)
...
if __name__ == "__main__":
    ...
    df_geometry = generate_weighted_sobol(n_samples=4096, **beta_distributions)
    
    df_geometry["wing_ID"] = df_geometry.apply(
    ...
    cols = ["wing_ID"] + [c for c in df_geometry.columns if c != "wing_ID"]
    df_geometry = df_geometry[cols]

    fig = plot_sampling_validation(df_geometry, beta_distributions)
    fig.show()
    ...
    generator.run(
        settings=settings,
        system_iter=wing_generator(df_geometry),
        total_systems=len(df_geometry),
