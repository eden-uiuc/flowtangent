...
def func_segmented_main_wing(
    ...
    small_jumps = np.isclose(np.diff(segment_root_chord_percents), 0.0) * np.isclose(
        np.diff(segment_thickness_to_chord), 0.0
    )
    ...
    # Root Chord
    rc = np.array([segment_root_chord_percents[i] for i in similar_segments])[1:] * wing_root_chord

    # Thickness to Chord
    tc = np.array([segment_thickness_to_chord[i] for i in similar_segments])[1:]

    # Previous Segments Quarter Chord Sweeps
    sw = np.array([segment_quarter_chord_sweeps[i] for i in similar_segments])[:-1]

    # Percent Span Locations
    sp = np.array([segment_percent_span_locations[i] for i in similar_segments])[1:]

    # Previous Segments Percent Span Locations
    sr = np.array([segment_percent_span_locations for i in similar_segments])[:-1]
        ...
        np.real(
            (
                ...
                * 1
                / 3
                * (
                    span_factor(sp) - span_factor(sr)
                )  # Close Span Factors Calculation  # Close Sweep Factor Calculation
            )
    ...
    # Root Chord
    drc = np.array([segment_root_chord_percents[i] for i in dissimilar_segments])[1:] * wing_root_chord

    # Previous Segments Root Chord
    drr = np.array([segment_root_chord_percents[i] for i in dissimilar_segments])[:-1] * wing_root_chord

    # Thickness to Chord
    dtc = np.array([segment_thickness_to_chord[i] for i in dissimilar_segments])[1:]

    # Previous Segments Thickness to Chord
    dtr = np.array([segment_thickness_to_chord[i] for i in dissimilar_segments])[:-1]

    # Previous Segments Quarter Chord Sweeps
    dsw = np.array([segment_quarter_chord_sweeps[i] for i in dissimilar_segments])[:-1]

    # Percent Span Locations
    dsp = np.array([segment_percent_span_locations[i] for i in dissimilar_segments])[1:]

    # Previous Segments Percent Span Locations
    dsr = np.array([segment_percent_span_locations[i] for i in dissimilar_segments])[:-1]
    ...
    def span_integral(x, A=drt, B=dts, C=dsr) -> np.ndarray:
        ...
        res = (1 / (4 * Bcb)) * (
            pi * x * (2 * Asq + 4 * ABC + Bsq * Csq2C)
            - xsqC * (2 * Q2 + 2 / 3 * B) * (3 * A * x + B * (-1 + 3 * C * x + xsq))
            + xas * (2 * B * ApBC - 2 * Q2 * x)
            - (4 * ApBC**2 * Q * V) / B
            - (Q3 * V) / Q
            + (8 * ApBC**2 * Q * V2) / B
            + (Q3 * V3) / Q
            - 4 * ApBC / B * (ApBC * Q * V4 + Qsq * V5)
        )

        return res

    dissimilar_segment_sum = np.sum(np.real((span_integral(dsp) - span_integral(dsr) / np.cos(dsw**2))))
...
def segmented_main_wing(
    ...
    main_wing_mass = func_segmented_main_wing(
        wingspan,
        wing_reference_area,
        wing_root_chord,
    ...
    wing.mass_properties.total = main_wing_mass * (1.0 - settings.mass_reduction_factors.main_wing)
