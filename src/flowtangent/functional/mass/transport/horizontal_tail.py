# flowtangent/Library/Methods/Mass/Correlation/Transport/horizontal_tail.py
...
def horizontal_tail(state: "rcf.state", system: "rcf.systems", settings: "rcf.settings"):
    ...
    moment_arm_length = (
        h_tail.origin[0] + h_tail.aerodynamic_center[0] - (main_wing.origin[0] + main_wing.aerodynamic_center[0])
    )

    h_tail_mass = func_horizontal_tail(
        wingspan,
        quarter_chord_sweep,
        thickness_to_chord,
        reference_area,
    ...
    h_tail.mass_properties.total = h_tail_mass * (1.0 - settings.mass_reduction_factors.empennage)
